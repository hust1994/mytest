...
public class CloudData extends BaseTest {

    /**
     * 2.访问网页，添加书签，tab，获得对应的书签，历史，tab数量，并同步数据，进入cloud data 看相应数据是否和已获得的数据对应相等
     * 点击Stop sync and erase data按钮，弹出提示框：It will stop syncing and clear all
     * cloud data.
    ...
    public void test_CloudData() throws Exception {
        ...
        TabPush.menuClickseeting1(0, solo);// 首页点击设置
        solo.sleep(Resource.TIME_SMALL);
...
//        solo.sleep(Resource.TIME_SMALL);
        ...
        solo.drag(screenWidth/3, screenWidth/3, screenHeight/4, screenHeight*3/4, 20);
        solo.sleep(2000);
        // 书签列表
        ...
        if (bookmarkNum < 6) {// 判断安装浏览器是否自带书签，如果没有，添加两个书签
            ...
            solo.clickOnView(solo.getViewByPath("main_menubar_holder[0][3]"));//
            // 点击网页页dolphin按钮
            solo.sleep(Resource.TIME_SMALL);
            v = solo.getText("Add");
            v = (View) v.getParent();
        ...
        solo.drag(screenWidth/2, screenWidth/2, screenHeight/4, screenHeight/2, 10);// 拖出tab
        solo.sleep(Resource.TIME_SMALL);
        realContent = (ViewGroup) TabPush.getAllViewByClassName("d", this.solo).get(1);// 添加tab的加号

        View view = realContent.getChildAt(1);
        // 添加2个tab，共3个
        for (int i = 0; i < 2; i++) {
        ...
        // 进入cloud data
        solo.sleep(Resource.TIME_SMALL);
        TabPush.menuClickseeting2(0, solo);// 网页点击设置
        solo.sleep(Resource.TIME_SMALL);
        solo.clickOnText("Dolphin Connect");
        solo.sleep(Resource.TIME_SMALL);
